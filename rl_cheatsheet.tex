...
\subsection{Return}
The \emph{return} is a some specific function of reward sequence. When there is a natural notion of final time step ($T$), the agent-environment interaction breaks naturally into sub-sequences (\emph{episodes}). Each episodes ends in a special state called \emph{terminal state}. $\mathcal{S}^{+}$ is the set of all states plus the terminal state.

The \emph{total discounted return} is expressed as the sum of rewards (opportunely discounted with $\gamma$):
\begin{align*}
...
\end{align*}
Where $\gamma$ is the \emph{discount factor} and $T$ is the final time step. It can be infinite.
When there is a natural notion of final time step, we have the \emph{episodes}.
...
Informally, is the expected return when starting from $s$, taking action $a$ and following $\pi$
\begin{align*}
...
If the environment's dynamic is completely known we can use \ref{eq:v_bellman} solving the system of $|\mathcal{S}|$ equations in  $|\mathcal{S}|$ unknowns ($v_\pi(s), s \in \mathcal{S}$). \\
...
\caption{Policy Iteration - estimating $\pi \sim \pi_*$ - [§4.3]}
\end{algorithm}
